...
const MainLayout = ({ children }) => {
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-darker overflow-hidden text-white font-sans selection:bg-primary selection:text-white">
      ...
      {!isPlayerPage ? (
        <div className="flex flex-1 overflow-hidden relative">
          ...
          <main className="flex-1 overflow-y-auto relative z-10 flex flex-col">
            ...
            <div className="relative z-10 w-full flex-1 p-8 bg-gradient-to-b from-darker/50 to-darker">
              {children}
            </div>
          </main>
        </div>
      ...
      <div className={`transition-all duration-500 ease-in-out ${currentTrack && !isPlayerPage ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0 absolute bottom-0 w-full'}`}>
